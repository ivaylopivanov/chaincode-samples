package main

import (
	"github.com/hyperledger/fabric/core/chaincode/shim"
	pb "github.com/hyperledger/fabric/protos/peer"
	"github.com/ivaylopivanov/chaincode-samples/storage/codes"
)

// Storage object
type Storage struct {
}

// Init will do nothing
func (s *Storage) Init(stub shim.ChaincodeStubInterface) pb.Response {
	args := []string{
		"1234567",
		`-----BEGIN PUBLIC KEY-----

cQIDAQAB
-----END PUBLIC KEY-----`,
		""}
	return s.create(stub, args)
}

// Invoke Storage method
func (s *Storage) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	fn, args := stub.GetFunctionAndParameters()
	switch fn {
	case "ping":
		return s.ping(stub, args)
	case "get":
		return s.get(stub, args)
	case "batchGet":
		return s.batchGet(stub, args)
	case "set":
		return s.set(stub, args)
	case "batchSet":
		return s.batchSet(stub, args)
	case "batchExchange":
		return s.batchExchange(stub, args)
	case "create":
		return s.create(stub, args)
	case "getKeys":
		return s.getKeys(stub, args)
	case "getPublicKey":
		return s.getPublicKey(stub, args)
	case "getPrivateKey":
		return s.getPrivateKey(stub, args)
	case "history":
		return s.history(stub, args)
	case "timeOfLastUpdate":
		return s.timeOfLastUpdate(stub, args)
	case "getDetailsForProperty":
		return s.getDetailsForProperty(stub, args)
	case "isVerified":
		return s.isVerified(stub, args)
	case "verify":
		return s.verify(stub, args)
	case "identify":
		return s.identify(stub, args)
	case "getVerifications":
		return s.getVerifications(stub, args)
	case "getVerificationFor":
		return s.getVerificationFor(stub, args)
	default:
		return shim.Error(codes.UnsupportedOperation)
	}
}
